    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          <h1 className="text-3xl font-bold text-gray-900 mb-6">Weight Log</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <p className="text-2xl font-bold text-green-900">
                  {entries[0]?.weight.toFixed(1)} lbs
                </p>
              </div>
              <div className="bg-purple-50 p-4 rounded-lg">
                <h3 className="text-sm font-medium text-purple-600">Starting Weight</h3>
                <p className="text-2xl font-bold text-purple-900">
                  {entries[entries.length - 1]?.weight.toFixed(1)} lbs
                </p>
              </div>
              {weightChange && (
                <div className={`p-4 rounded-lg ${
                  weightChange.change > 0 ? 'bg-red-50' : weightChange.change < 0 ? 'bg-green-50' : 'bg-gray-50'
                }`}>
                  <h3 className={`text-sm font-medium ${
                    weightChange.change > 0 ? 'text-red-600' : weightChange.change < 0 ? 'text-green-600' : 'text-gray-600'
                  }`}>
                    Total Change
                  </h3>
                  <p className={`text-2xl font-bold ${
                    weightChange.change > 0 ? 'text-red-900' : weightChange.change < 0 ? 'text-green-900' : 'text-gray-900'
                  }`}>
                    {weightChange.change > 0 ? '+' : ''}{weightChange.change.toFixed(1)} lbs
                  </p>
                  <p className={`text-sm ${
                    weightChange.change > 0 ? 'text-red-600' : weightChange.change < 0 ? 'text-green-600' : 'text-gray-600'
                  }`}>
